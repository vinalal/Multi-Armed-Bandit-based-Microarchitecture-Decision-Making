...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // find victim
        // mshr se data nikal ke cache me bharna hai
        // pehle data ke address se set nikalte hain
        ...
        // page fault hai toh kya hi farak padta hai
        ...
        // Replace karne se pehle victim block ko lower level me bhejte hai
        if ((block[set][way].valid && (cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C)) ||
                (block[set][way].dirty && !(cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C))) {
        ...
        if (do_fill){
            ...
            if ( (cache_type!=IS_DTLB) || (cache_type==IS_DTLB && MSHR.entry[mshr_index].type != PREFETCH_TRANSLATION) )
                ...
                // Abhi hum MSHR se data nikal ke cache me bharenge
                // toh pehle jo data evict ho raha hai usko lower level me daalte hain
                // agar dirty hai toh pehle hi kar chuke hain
                // Idhar pe humne naya data (jo MSHR me tha) cache me bhar diya
                fill_cache(set, way, &MSHR.entry[mshr_index]);

                // Ab lower cache se wo data hatana bhi hoga kyuki exclusive hai
                if(lower_level && (cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C))
                {
                    CACHE* lower_level_cache = dynamic_cast<CACHE*>(lower_level);
                    if(lower_level_cache == nullptr)
                    {   
                        std::cout << "lower level is not a cache, it is something else" << std::endl;
                        assert(0);
                    }
                    lower_level_cache->invalidate_entry(MSHR.entry[mshr_index].address);
                }
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                {
                    // miss
                    l2c_prefetcher_operate(WQ.entry[index].address<<LOG2_BLOCK_SIZE, WQ.entry[index].ip, 0, WQ.entry[index].type, UINT32_MAX, WQ.entry[index].critical_ip_flag);
                }
                ...
                if (do_fill) {
                    ...
                    if(lower_level && (cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C))
                    {
                        ...
                        lower_level_cache->invalidate_entry(WQ.entry[index].address);
                    }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                #ifdef SANITY_CHECK
                    // Exclusivity sanity checks: in an exclusive hierarchy,
                    if (cache_type == IS_L1D) {
                        // build a PACKET to query lower caches using a physical-like address

                        if (dynamic_cast<CACHE*>(lower_level)->check_hit(&RQ.entry[index]) >= 0) {
                            cerr << "SANITY_CHECK: exclusive L1 contains a line that also exists in L2 - addr: " << hex << RQ.entry[index].full_addr << dec << "\n";
                            assert(0);
                        }
                        // Check LLC
                        if (dynamic_cast<CACHE*>((dynamic_cast<CACHE*>(lower_level))->lower_level)->check_hit(&RQ.entry[index]) >= 0) {
                            cerr << "SANITY_CHECK: exclusive L1 contains a line that also exists in LLC - addr: " << hex << RQ.entry[index].full_addr << dec << "\n";
                            assert(0);
                        }
                    }
                    else if (cache_type == IS_L2C) {
                        // Check LLC
                        if (dynamic_cast<CACHE*>(lower_level)->check_hit(&RQ.entry[index]) >= 0) {
                            cerr << "SANITY_CHECK: exclusive L2 contains a line that also exists in LLC - addr: " << hex << RQ.entry[index].full_addr << dec << "\n";
                            cerr << "address " << RQ.entry[index].address << " full_addr " << RQ.entry[index].full_addr << " pa " << RQ.entry[index].full_physical_address << endl;
                            assert(0);
                        }
                    }
                ...
                if (RQ.entry[index].type == LOAD) {
                    ...
                    if (cache_type == IS_L1D) 
                        l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type, RQ.entry[index].critical_ip_flag);	// RQ.entry[index].instr_id);
                            ...
                            if (block[set][way].pf_metadata > 2*PAGE_SIZE){
                                cout << "Error: L2C prefetch metadata cannot be greater than page size"<<endl;
                                assert(0);
                            }
                            uint32_t metadata_in = block[set][way].pf_metadata << 8 | block[set][way].prefetch;
                            l2c_prefetcher_operate(block[set][way].address<<LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, metadata_in, RQ.entry[index].critical_ip_flag);
                        }
