...
//         }
        ...
        if (do_fill){
            ...
            if (block[set][way].valid && (cache_type == IS_L1D || cache_type == IS_L2C)) {
...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        {
            if (packet->full_addr == 0xFDFD3A7CEF58ULL) {
                cout << "Filling address " << hex << packet->full_addr << dec << " in cache level ";
                if (cache_type == IS_L1D)
                    cout << "L1D" << endl;
                else if (cache_type == IS_L1I)
                    cout << "L1I" << endl;
                    ...
                    cout << "STLB" << endl;
                else if (cache_type == IS_PSCL5)
                    cout << "PSCL5" << endl;
                else if (cache_type == IS_PSCL4)
                    cout << "PSCL4" << endl;
                else if (cache_type == IS_PSCL3)
                    cout << "PSCL3" << endl;
        ...
        int CACHE::invalidate_entry(uint64_t inval_addr)
        {
            ...
            for (uint32_t way=0; way<NUM_WAY; way++) {
                if (block[set][way].valid && (block[set][way].tag == inval_addr)) {
                    ...
                    match_way = way;
                    if (inval_addr == 0xFDFD3A7CEF58ULL) {
                        cout << "Invalidating address " << hex << inval_addr << dec << " in cache level ";
                        if (cache_type == IS_L1D)
                            cout << "L1D" << endl;
                        else if (cache_type == IS_L1I)
                            cout << "L1I" << endl;
