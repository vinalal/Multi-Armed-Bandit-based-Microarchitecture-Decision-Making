...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // ab victim find karte hain jo evict hoga
            ...
            if (way >= 0) { // read hit
                ...
                if (RQ.entry[index].type == LOAD) {
                    ...
                    if (cache_type == IS_L1D) 
                        ...
                        // cout << "offset " << block[set][way].pf_metadata << " and prefetch " << static_cast<uint32_t>(block[set][way].prefetch) << " from L2C cache line" << endl;
                        ...
                        // cout << "Sending offset " << block[set][way].pf_metadata << " and prefetch " << static_cast<uint32_t>(block[set][way].prefetch) << " as metadata " << metadata_in << " to L2C prefetcher" << endl;
